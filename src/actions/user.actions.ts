...
export async function updateProfileAction(data: any) {
  try {
    ...
    // Update in Database
    await User.findByIdAndUpdate(payload._id, { $set: result.data });
...
export async function changePasswordAction(formData: any) {
  try {
    ...
    // Database se user lo (Password explicitly mangwao)
    const user = await User.findById(payload._id).select("+password");
    if (!user) return { success: false, message: "User not found" };

    // SECURITY: Purana password check karo
    ...
    await user.save(); // Model hook will auto-hash this 
